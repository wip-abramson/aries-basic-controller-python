...
# REMOVE INDY FROM IMPORT
# from aries_basic_controller.utils import (
#     log_json,
#     log_msg,
#     log_status,
#     log_timer,
...
async def input_invitation(agent):
    ...
    with log_timer("Connect duration:"):
        # connection = await agent.connections_controller.admin_POST("/connections/receive-invitation", details)
        # agent.connection_id = connection["connection_id"]
        # log_json(connection, label="Invitation response:")

        connection = await agent.connections_controller.receive_invitation(details)
        agent.connection_id = connection["connection_id"]
        log_json(connection, label="Invitation response:")

        accept = await agent.connections_controller.accept_request(agent.connection_id)
        log_json(accept, label="Invitation Accepted:")
...
async def main(start_port: int, no_auto: bool = False, show_timing: bool = False):
    ...
    try:
        ...
        #     timing=show_timing,
        # )
        agent = AriesAgentController(webhook_host=RESEARCHER_WEBHOOK_HOST, webhook_port=RESEARCHER_WEBHOOK_PORT,
                                               webhook_base=RESEARCHER_WEBHOOK_BASE, admin_url=RESEARCHER_ADMIN_URL, connections=True)

        # await agent.listen_webhooks(start_port + 2)
        await agent.listen_webhooks(RESEARCHER_WEBHOOK_PORT)
